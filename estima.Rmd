...
Una estimación puntual de un parámetro $\theta$ es un número único que puede ser considerado como un valor sensible de $\theta$. Se obtiene una estimación puntual seleccionando un estadístico apropiado y calculando su valor con los datos muestrales dados. El estadístico seleccionado se llama estimador puntual de $\theta$.
...
```{r fig.asp=0.8, fig.align='center', echo=FALSE}
knitr::include_graphics("imagen/est.png")
```
...
Solución Utilizaremos el estimador $\hat p=y/n$ para calcular p. Por tanto, la estimación de p, la fracción
de votantes que están a favor del candidato petro, es
...
Como n = 1000, cuando $b = 2 \sigma_{\hat p}$, la probabilidad de que e sea menor que b es aproximadamente
0.95.
