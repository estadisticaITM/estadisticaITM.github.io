...
##Antes de hacer un modelo de egresión lineal
...
$$\hat{\beta_1}=\frac{S_{xy}}{S_{xx}}=\frac{cov(x,y)}{S_{xx}}=\frac{\sum_{i=1}^{n}(x_i-\bar{x})(y_i-\bar{y})}{\sum_{i=1}^{n}(x_i-\bar{x})^2}$$

A las cantidades $S_{xx}$ y $S_{xy}$ se les conoce como suma corregida de cuadrados y suma corregida de productos cruzados de x y y, respectivamente
...
$$SSE=\sum_{i=1}^{n}(y_i-\hat{y_i})^2=\sum_{i=1}^n e^2_i$$
...
$$MSE=\frac{SSE}{n-2}=\frac{\sum_{i=1}^{n}(y_i-\hat{y_i})^2}{n-2}=\frac{\sum_{i=1}^n e^2_i}{n-2}$$

Se puede demostrar que MSE, es un estimador insesgado de $\sigma^2$ para el modelo de RLS, es decir que:
...
$MST=\frac{SST}{n-1}$: Cuadrado medio total

$MSR=\frac{SSR}{1}$:Cuadrado medio de la regresión

$MSE=\frac{SSE}{n-2}$: Cuadrado medio del error
...
Hipótesis nula:   $H_0: \beta_i=0$ el parámetro no es significativo

Hipótesis alternativa:  $H_1: \beta_i \not= 0$: el parámeto es significativo

El estadístico de prueba corresponde a 

$$F_c=\frac{MSR}{MSE}\sim F_{(1,n-2)}$$

##Desición

Rechazar $H_0$ si $F_c>F_{(1,n-2)}$
...
$${\hat{\sigma}}^2=\frac{\sum_{i=1}^n {e}_i^2}{n}$$
...
- $\beta_1$ es el valor de la pendiente, es decir que por cada unidad que aumente la variable independiente, la variable dependiente aumenta $\beta_1$ unidades
...
## Modelo de regresión lineal con variables indicadoras
...
Sucede cuando son mas de 2 variables regresoras