...
$$Y_i=\beta_0+\beta_1 X_{i1}+\beta_2 X_{i2}+\beta_3X_{1}X_2+\varepsilon_i$$
...
$$Y_i=\beta_0+\beta_1 X_{i1}+\beta_2 X_{i2}+\beta_3X_{3}+\varepsilon_i$$
...
El objetivo del análisis es determinar cuál es la relación entre dichas medidas y el volumen
de madera, con el fin de poder predecir este último en función de las primeras
...
En el primer caso es necesario considerar la interacción entre $X_1$ y $X_2$ en el modelo de regresión, y sólo utilizamos c − 1 de las posibles variables indicadoras de las categorías de la
variable $X_2$, es decir postulamos el modelo:

$$\large Y=\beta_0 +\beta_1 I_1+\beta_3I_2+\beta_{1,1}X_1I_1+\beta_{1,2}X_1I_2+\varepsilon$$
...
$$\large Y=\beta_0 +\beta_1X_1+\beta_2I_2+\beta_3I_3+ ... \varepsilon$$
...
- Si $I_1=1$, entonces el resto de indicadoras son iguales a cero y obtenemos,
$$\large Y=(\beta_0+\beta_2)+\beta_1X_1+\varepsilon$$
...
Con base en la tabla ANOVA, y bajo los supuestosde los errores, se realiza el test de significancia de la regresión el cual se enuncia de la siguiente manera:
$H_0= \beta_1=\beta_2=...\beta_k$ El modelo de regresión no es significativo.
$H_1=Algún \quad \beta_k \not=0 $Existe una relación de regresión significativa con al menos una de las variables.
