...
|Tipo de residuales|Definición |Fórmula|Función en R|
|:------|:------------------|:------:|:------------:|
...
|**Estudentizados** También se les conoce como jackknife o residuos eliminados|se calculan utilizando una estimación de la desviación estándar del modelo, donde se elimina la observación actual. |$$ r_i=\frac{\hat \varepsilon_i}{\sqrt{MSE(1-h_{ii})}}$$ $h_{ii}$ **leverage** es una medida de ubicación del i-ésimo punto residual|rstandard(mod)|
...
**Punto Distante:** Punto con un valor inusual o extremo para la variable independiente (X), en comparación con el resto de los datos.

**Influencia Potencial:** El apalancamiento mide el potencial que tiene esa observación para influir o "jalar" la línea de regresión hacia sí misma.

**Solo depende de X:** depende únicamente de los valores de las variables predictoras (X), no del valor observado de la variable dependiente (Y).
...
El leverage es diferente de otros conceptos como:

**Outlier (Valor Atípico)**tiene un valor Y (variable dependiente) inusual o extremo, resultando en un residuo grande (gran diferencia entre el valor observado y el predicho). Puede tener leverage bajo o alto.

**Punto Influyente o de balanceo** Punto que si se elimina del conjunto de datos, cambiaría los coeficientes (pendiente e intercepto) del modelo de regresión. Puede tener un leverage alto como un residuo grande.
...
### estimación de los tipos de residuales
