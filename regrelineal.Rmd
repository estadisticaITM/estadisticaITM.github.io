...
|Tipo de residuales|Definición |Fórmula|
|:------|:---------------------|:------:|
|**Ordinarios**|Diferencia de cada valor $y_i$ de la variable respuesta y su estimación $\hat{y_i}$ |$e_i= y_i- \hat{y_i}$|
|**Estandarizados**|Se calculan dividiendo el residual por su desviación estándar (MSE)|$$\large d_i=\frac{\hat \varepsilon_i}{\sqrt{MSE}}$$|
|**Estudentizados** 
También se les conoce como jackknife o residuos eliminados|se calculan utilizando una estimación de la desviación estándar del modelo, donde se elimina la observación actual. |$$ r_i=\frac{\hat \varepsilon_i}{\sqrt{MSE(1-h_{ii})}}$$ $h_{ii}$ **leverage** es una medida de ubicación del i-ésimo punto residual|
|**Pearson**|Se utilizan en modelos lineales generalizados, como la regresión logística y de Poisson |$$r_i=\frac{obs_i-esp_i}{\sqrt{var\quad esp_i}}$$|
...
Un punto con un alto apalancamiento es un punto de datos que tiene un valor inusual o extremo para la(s) variable(s) independiente(s) (X), en comparación con el resto de los datos.

**Influencia Potencial:**

El apalancamiento mide el potencial que tiene esa observación para influir o "jalar" la línea de regresión hacia sí misma.

**Solo depende de X:**

El apalancamiento de un punto depende únicamente de los valores de las variables predictoras (X), no del valor observado de la variable dependiente (Y).

**Interpretación **

**Alto Leverage:** 

Se encuentra geográficamente lejos del "centro de gravedad" de los datos en el espacio de las variables X. Estos puntos tienen un peso desproporcionado en la determinación de la pendiente de la línea de regresión.

**Bajo Leverage:** 

Un punto con bajo leverage está cerca de la media de las variables X y tiene un impacto mínimo en la forma final de la línea de regresión.

**Diferencia con "Outlier" e "Influential Point"**

Es importante diferenciar el leverage de otros conceptos:

**Outlier (Valor Atípico)** Un outlier tiene un valor Y (variable dependiente) inusual o extremo, resultando en un residuo grande (gran diferencia entre el valor observado y el predicho). Puede tener leverage bajo o alto.

**Punto Influyente** 

Es un punto que, si se elimina del conjunto de datos, cambiaría significativamente los coeficientes (pendiente e intercepto) del modelo de regresión. Un punto influyente suele tener tanto un leverage alto como un residuo grande.
...
son útiles para identificar valores atípicos, puntos influyentes y comprobar los supuestos del modelo, en general un residual grande, es cuando su valor es superior a 3 $(|di| > 3)$, lo cual  es indicio de una observación atípica potencial.
Son más eficaces para detectar puntos de datos influyentes.
